...
- Abuse limits (no new dependencies): global cap of 20 new reviews/hour (DB count), owner login locked 15 min after 20 failures (`/admin/login/` redirects to the throttled page), POST bodies capped at 64 KB, HSTS in prod. Volumetric DDoS protection = Cloudflare proxy + rate-limit rule in front of the API, and `reviews.middleware.origin_auth` refuses requests without Cloudflare's secret `X-Origin-Auth` header when `ORIGIN_AUTH_SECRET` is set (deploy handoff Step 4b). The API URL cannot be hidden: the browser must call it.
